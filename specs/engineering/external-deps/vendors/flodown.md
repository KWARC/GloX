...
- Shared rewrite: `src/lib/prepareFloDownStatement.ts` — pass stored HTTP symbol URIs (D-FTML-05). Preview and sTeX export register declarations via `addSymbolDeclaration(symbolName)` on the declaring document without rewriting `uri` fields.
